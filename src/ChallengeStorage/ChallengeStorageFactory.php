...
class ChallengeStorageFactory
{
    ...
    /**
     * @param array $config 存储配置，格式：['type' => 'session'] 或 ['type' => 'redis', 'ttl' => 300, 'connection' => 'default']
     */
    public function create(array $config, int $challengeTtl = 300): ChallengeStorageInterface
    {
        ...
        return match ($type) {
            'session' => new SessionChallengeStorage(
                $this->container->get(\Hyperf\Contract\SessionInterface::class)
            ),
            'redis' => new RedisChallengeStorage(
                $this->container->get(\Hyperf\Redis\Redis::class),
                $config['ttl'] ?? $challengeTtl,
            ),
            default => throw new InvalidArgumentException(
                sprintf('Unsupported 2FA challenge storage type: "%s". Supported types: session, redis.', $type)
            ),
        };
    }
}
